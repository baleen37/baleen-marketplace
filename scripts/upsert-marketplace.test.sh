...
echo '{"name":"episodic-memory","version":"1.11.3","path":"."}' \
  | SOURCES_JSON="$TMP/sources.json" bash "$SCRIPT" episodic-memory "$TMP/mp.json"
src=$(jq -r '.plugins[] | select(.name=="episodic-memory") | .source.source' "$TMP/mp.json")
[[ "$src" == "url" ]] || { echo "FAIL: episodic-memory source.source=$src (expected url)"; exit 1; }
url=$(jq -r '.plugins[] | select(.name=="episodic-memory") | .source.url' "$TMP/mp.json")
[[ "$url" == "https://github.com/baleen37/episodic-memory.git" ]] || { echo "FAIL: episodic-memory source.url=$url"; exit 1; }

echo '{"name":"episodic-memory","version":"1.11.4","path":"."}' \
  | SOURCES_JSON="$TMP/sources.json" bash "$SCRIPT" episodic-memory "$TMP/mp.json"
src=$(jq -r '.plugins[] | select(.name=="episodic-memory") | .source.source' "$TMP/mp.json")
[[ "$src" == "url" ]] || { echo "FAIL: updated episodic-memory source.source=$src (expected url)"; exit 1; }
url=$(jq -r '.plugins[] | select(.name=="episodic-memory") | .source.url' "$TMP/mp.json")
[[ "$url" == "https://github.com/baleen37/episodic-memory.git" ]] || { echo "FAIL: updated episodic-memory source.url=$url"; exit 1; }
has_repo=$(jq '.plugins[] | select(.name=="episodic-memory") | .source | has("repo")' "$TMP/mp.json")
[[ "$has_repo" == "false" ]] || { echo "FAIL: updated episodic-memory root entry should have no repo"; exit 1; }

echo "PASS: upsert-marketplace (claude)"

# ── Codex FORMAT=codex ──────────────────────────────────────────────────────
# 신규 엔트리: episodic-memory (path=".") → Codex는 루트 repo source를 "url"로 인식한다
cat > "$TMP/codex-mp.json" <<'EOF'
{"name":"baleen-marketplace","plugins":[]}
EOF

echo '{"name":"episodic-memory","version":"1.11.4","path":"."}' \
  | SOURCES_JSON="$TMP/sources.json" FORMAT=codex bash "$SCRIPT" episodic-memory "$TMP/codex-mp.json"
...
[[ "$src_url" == "https://github.com/baleen37/episodic-memory.git" ]] || { echo "FAIL: codex url=$src_url"; exit 1; }
...
{"name":"baleen-marketplace","plugins":[
  {"name":"episodic-memory","source":{"source":"url","url":"https://github.com/baleen37/episodic-memory.git"},"version":"1.11.3","category":"Productivity","policy":{"installation":"AVAILABLE","authentication":"ON_INSTALL"}}
]}
EOF

echo '{"name":"episodic-memory","version":"1.11.4","path":"."}' \
  | SOURCES_JSON="$TMP/sources.json" FORMAT=codex bash "$SCRIPT" episodic-memory "$TMP/codex-mp2.json"
...
EOF
  ...
  episodic_memory_version=$(jq -r '.plugins[] | select(.name=="episodic-memory") | .version' "$TMP/removal-$format.json")
